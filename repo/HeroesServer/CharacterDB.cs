...
namespace ServerUtilities
{
    public class CharacterDB
    {
        ...
        private static void SaveCharacterStats(List<Character> players)
        {
            ...
            foreach (var player in players)
            {
                sqlItems += string.Format("({0}, {1}, {2}, {3}, {4}, {5}, {6}, {7}),", player.DatabaseId, player.GetStatValue(StatType.HEALTH),
                            DatabaseManager.FloatToDouble(player.Position.X), DatabaseManager.FloatToDouble(player.Position.Y), DatabaseManager.FloatToDouble(player.Position.Z), 
                            player.GetStatValue(StatType.LEVEL), player.GetStatValue(StatType.EXPERIENCE), player.GetStatValue(StatType.SKILL_POINTS));
            }
            ...
            string sql = string.Format("INSERT INTO characters ({0}) VALUES {1} ON DUPLICATE KEY UPDATE health=VALUES(health), pos_x=VALUES(pos_x), pos_y=VALUES(pos_y), pos_z=VALUES(pos_z), level=VALUES(level), " +
                        "experience=VALUES(experience), skill_points=VALUES(skill_points)", "id, health, pos_x, pos_y, pos_z, level, experience, skill_points", sqlItems);
